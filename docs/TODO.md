...
| M16-T01 | إصلاح Analyze الحي: `analyze.set_mode` + حقول حسب النمط + تعريب الاختيارات + أخطاء مترجمة بدل `err.unknown` (من ملف M16 MASTER — المرجع الوحيد بقرار AUTHORITY) | VERIFIED COMPLETE | PHASE_M16_T01 · بوابة T01 `97 passed` · كامل `443 passed` · launcher `26/42 ready` · notebooks identical · package build OK · PR #23 مُدموج على main (`4a2dac6`) · خطة M16 اللاحقة حلّ محلها M17 MASTER |
...
| M17-T02 | إصلاح كل action حسب أولويات M17 (Drive → Dashboard → Transfers-proof → Logs → Settings → Export/Recovery) | PLANNED — بانتظار موافقة Brain على جرد T01 | العوائق موثقة في `KNOWN_ISSUES #27–#30` وفي عمودي `current_blocker`/`recommended_fix` بالـinventory |
