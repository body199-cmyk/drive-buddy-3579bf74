...
| M10-T02 | توحيد هوية v4.5.0 **في CI** | BLOCKED — بسبب منصة | إصلاح `ci.yml` (v4.5) مجهَّز ومتحقق محليًا؛ الدفع مُنع: GitHub App بلا صلاحية `workflows`. **ولحقت به أولوية إصلاح #13**: CI لا يبدأ أصلًا (`runner.temp` في job-env — فشل التهيئة على كل الفروع). يُغلق عند هبوط الأسطر الثلاثة الموثقة في PHASE_12 في main |
