...
| 8 | **CI كان يبني `teledrive_v3.1.zip`** بينما `AI_HANDOFF` و`TODO #2` ادّعيا تحديثه إلى v4.5 | شجرة `.github` عند HEAD 4cacc58 = `86abde40`، مطابقة لما قبل PR#3؛ blob `ci.yml` = `5dec51e` | **مفتوحة** — الحل مجهَّز ومتحقق محليًا (3 أسطر موثقة في PHASE_12 وPHASE_13)؛ بانتظار تطبيق المالك — قيد صلاحية `workflows`. يُغلق عند الهبوط في main |
...
| 13 | **CI لا يبدأ أصلًا (Invalid workflow file) على كل الفروع بما فيها main**: التعليق التوضيحي على run 31241947281 = `Invalid workflow file: .github/workflows/ci.yml#L1 (Line: 16, Col: 23): Unrecognized named-value: 'runner'. Located at position 1 within expression: runner.temp` — سياق `runner` غير متاح في `jobs.<id>.env` | runs 31129230384 (دفعة المالك نفسه، 1 يوم)، 31237246480 (PR#2 merge)، 31239298725 (PR#3 merge)، 31241771183/31240323871 (PR#4 merge) كلها 0s بنفس الفشل؛ آخر run أخضر = 30496659877 بملف blob 1caddeb الذي كان يستخدم `${{ github.workspace }}` | **مفتوحة** — مكتشَفة في M12-T01. **السبب الجذري:** سياق `runner` غير متاح في `jobs.<id>.env`؛ المتاح `github/needs/strategy/matrix/vars/secrets/inputs` فقط. النتيجة ملف workflow غير صالح وفشل في صفر ثانية في **كل** تشغيل منذ إدخال السطر. الأثر: بوابات §16 لم تُشغَّل على GitHub ولا مرة. الإصلاح (3 أسطر موثقة في PHASE_13) بانتظار تطبيق المالك — قيد صلاحية `workflows` |
| 14 | **تلوث بيت الذاكرة:** commit المالك `afde5fe` أنشأ `docs/pic for frontend` (بايت واحد، بلا محتوى أو مرجع) | `ls -la "docs/pic for frontend"` = 1 بايت؛ لا مرجع في أي ملف | ✅ حُذف في M12-T02 |
| 15 | **صلاحية المنصة:** تطبيق GitHub الخاص بـArena لا يملك `workflows`، فلا يستطيع تسليم أي تعديل على `.github/workflows/*` | `git push` → remote rejected refusing to allow a GitHub App to create or update workflow; REST API → HTTP 403 Resource not accessible by integration (PHASE_12) | **مفتوحة بنيويًا** — المخرج هو تطبيق المالك اليدوي. هذا يفسر أيضًا لماذا لم يصل تعديل CI المزعوم في PR #3 إطلاقًا |
