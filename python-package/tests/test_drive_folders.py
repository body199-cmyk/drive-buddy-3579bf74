...
def test_create_folder_action_validates_name_and_parent(ctx):
    ...
    for bad in ("", "   "):
        message, created = ctx.handlers.h_drive_create_folder(bad, "root")
...
def test_select_folder_action_validates_mimetype_and_stores_the_id(ctx):
    ...
    message, stored = ctx.handlers.h_drive_select_folder("notes.txt :: id_plain")
    ...
    message, stored = ctx.handlers.h_drive_select_folder("Alpha :: id_alpha")
    assert message == t("msg.folder_selected")
    assert stored == "id_alpha"
    ...
    assert ERROR_ARITY["drive.select_folder"] == 2
